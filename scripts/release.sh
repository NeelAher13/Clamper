...
PROJECT_NAME="Clamper"
PRODUCT_NAME="Clamper"
GITHUB_REPO="validatedev/Clamper"
NOTARIZATION_PROFILE="Clamper-Notarization"
...
print_usage() {
    ...
    echo ""
    echo "Arguments:"
    echo "  version   Version string (e.g., 1.0.0, 1.1.0)"
    echo ""
    echo "Example:"
    echo "  $0 1.0.0"
    echo ""
...
update_homebrew_cask() {
    ...
    local cask_file="$PROJECT_ROOT/Casks/clamper.rb"
...
update_tap_repo() {
    ...
    local cask_source="$PROJECT_ROOT/Casks/clamper.rb"
    ...
    cd "$tap_dir"
    git add Casks/clamper.rb
    if git diff --cached --quiet; then
        echo "Tap already up to date."
    else
        git commit -m "chore: update clamper to $version"
...
submit_homebrew_pr() {
    ...
    if [ ! -f "$PROJECT_ROOT/Casks/clamper.rb" ]; then
        echo "Cask file not found at $PROJECT_ROOT/Casks/clamper.rb"
        return 1
    fi

    local audit_cask_path="$PROJECT_ROOT/Casks/clamper.rb"
    ...
    if ! brew bump-cask-pr clamper --version "$version"; then
        echo "Failed to submit Homebrew PR."
        echo "Fallback: publish the GitHub release, then run:"
        echo "  brew bump-cask-pr clamper --version $version"
        return 1
    fi
...
if [[ ! "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo "Error: Invalid version format '$VERSION'. Use semver (e.g., 1.2.3)"
    exit 1
fi
BUILD_DIR="$PROJECT_ROOT/build"
APP_PATH="$BUILD_DIR/Build/Products/Release/$PRODUCT_NAME.app"
CASK_FILE_PATH="$PROJECT_ROOT/Casks/clamper.rb"
...
if [[ "$HOMEBREW_PR_SUBMITTED" != "yes" ]]; then
    ...
    echo "  - Run: brew bump-cask-pr clamper --version $VERSION"
fi
echo ""
